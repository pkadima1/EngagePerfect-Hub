...
const Layout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      {/* Header */}
      <header className="sticky top-0 z-10 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 shadow-sm">
            ...
            <nav className="hidden md:flex space-x-8 items-center">
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <Link
                    to="/signup"
                    className="ml-4 px-4 py-2 rounded-md text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm"
                  >
                    Sign up
                  </Link>
                </>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-inner">
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <div className="px-3 py-2 text-sm text-gray-600 dark:text-gray-400">
                    Signed in as: {currentUser?.displayName || userData?.displayName || 'User'}
                  </div>
                  <button
                    onClick={() => {
                      handleLogout();
                    ...
                    className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400"
                  >
                    Sign out
                  </button>
                </>
                ...
                <>
                  ...
                  <Link
                    to="/signup"
                    className="block mt-2 px-3 py-2 rounded-md text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Sign up
                  </Link>
